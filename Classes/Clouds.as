...
package {
	...
	public class Clouds extends Sprite {
		...
		public function Clouds(w:int = 300, h:int = 200, scX:int = -1, scY:int = 2, useBG:Boolean = true, col:uint = 0x2255aa) {
			...
			matrix.createGradientBox(displayWidth, 350);
			mymask.graphics.lineStyle();
			mymask.graphics.beginGradientFill('radial', [0xFFFFFF, 0xFFFFFF],  [1, 0], [125, 255], matrix);
			mymask.graphics.drawRect(0, 0, displayWidth, 350);
			mymask.graphics.endFill();
			addChild(mymask);

			this.mask = mymask;

			// кэшируем объект и маску иначе работать не будет
